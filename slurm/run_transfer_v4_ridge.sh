...
#   slurm/run_transfer_v4_all.sh
#
# WHY A NEW RUN AT ALL (v4 vs v3)
# -------------------------------
# v3 fixed the normalisation (convention D: the map is fitted and applied in one
# coordinate system, and the source is normalised at Ls rather than at the align
# cache's own best layer).  v4 additionally fixes the EVAL/POOL SPLIT.
#
# Up to v3 the 500/1500 split was one permutation per (dataset, model), drawn from
# a single rng created outside the model loop.  Every cache was internally
# consistent, but across models the splits crossed: the target's 1500 pool rows
# contained ~368 of the source's 500 eval rows.  Since a cell is scored on the
# SOURCE cache's eval rows, best-to-best's and best-to-best-sub's supervised target
# layer choice had seen ~74% resp. ~7% of the eval questions with their labels.
# The split is now a pure function of (dataset, n_eval, seed) -- every model on a
# dataset holds out the same questions -- so that overlap is exactly zero.
#
# This invalidates EVERYTHING from v3 downwards, not just the two leaky modes:
# different eval rows and different pool rows change every curve in all four
# modes.  So v4 is built from scratch and shares nothing with any older tree --
# there is no seed script and no probe caches are copied in.  Two stamps make an
# accidental mix fail loudly instead of silently: SPLIT_CONVENTION on every probe
# cache (checked pairwise by _check_split_convention) and NORM_CONVENTION on every
# alignment cache (checked by _check_norm_convention).
#
...
#
# Usage:
#   MODE=best-to-best bash slurm/run_transfer_v4_ridge.sh
#   OUT_ROOT=/somewhere MODE=best-to-align bash slurm/run_transfer_v4_ridge.sh
#
# Env knobs:
