#!/bin/bash
# Driver: all four ridge transfer modes under normalisation convention D.
#
# Output tree (nothing is shared with the older transfer_v2* trees -- see
# run_transfer_v3_ridge.sh for why):
#   $OUT_ROOT/_probes_shared/probes   phase 1, built once
#   $OUT_ROOT/transfer_v3_b2b         best-to-best
#   $OUT_ROOT/transfer_v3_btl         best-to-last
#   $OUT_ROOT/transfer_v3_bbs         best-to-best-sub
#   $OUT_ROOT/transfer_v3_b2a         best-to-align
...
#
# Usage:
#   nohup bash slurm/run_transfer_v3_all.sh \
#     >> /proj/MR_dataset/mtk53728/UQ/sep_scratch/transfer_v3/run_all.log 2>&1 &
#
#   # one mode only, or a different order:
#   MODES="best-to-align" bash slurm/run_transfer_v3_all.sh
#
#   # everything except the plots:
#   SKIP_SUMMARY=1 bash slurm/run_transfer_v3_all.sh
...
MODES=${MODES:-"best-to-best-sub best-to-align best-to-last best-to-best"}
...
echo "##########################################"
echo "transfer v3 (convention D), ridge alpha=1e4"
echo "modes:    $MODES"
...
MODE=best-to-best OUT="$SHARED" PHASE1_ONLY=1 \
    bash slurm/run_transfer_v3_ridge.sh 2>&1 | tee "$OUT_ROOT/_probes_shared.log"
...
for mode in $MODES; do
    ...
    if MODE="$mode" bash slurm/run_transfer_v3_ridge.sh 2>&1 | tee "$log"; then
