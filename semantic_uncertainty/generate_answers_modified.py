    ...
    for dataset_split in ['train', 'validation']:
        ...
        for index in tqdm(indices):
            ...
            for i in range(num_generations):
                ...
                predicted_answer, token_log_likelihoods, (embedding, latent_embeddings) = model.predict(
                    local_prompt, temperature, return_latent=True)
                
                # Last-token embedding from the final layer.
                embedding = embedding.cpu() if embedding is not None else None

                # Dictionary of all-layer embeddings for requested token positions,
                # e.g. {'tbg': tensor, '0': tensor, '-2': tensor, '-1': tensor}.
                latent_embeddings = {
                    position: emb.cpu() if emb is not None else None
                    for position, emb in latent_embeddings.items()
                }
                ...
                if i == 0:
                    ...
                    full_responses.append(
                        (predicted_answer, token_log_likelihoods, embedding, latent_embeddings, acc))
